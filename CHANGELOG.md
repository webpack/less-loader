...
- Minimum supported Node.js version is now `>= 22.11.0`. (by [@alexander-akait](https://github.com/alexander-akait) in [#590](https://github.com/webpack/less-loader/pull/590))

- The package now ships native ESM as well as CommonJS. `"type": "module"` is set, and an `exports` map exposes `dist/esm/index.js` to `import` and `dist/cjs/index.js` to `require`. Less is loaded via dynamic `import()` instead of `require()`. (by [@alexander-akait](https://github.com/alexander-akait) in [#593](https://github.com/webpack/less-loader/pull/593))

- `lessLogAsWarnOrErr` now defaults to `true`. Less warnings and errors are emitted as webpack warnings and errors by default. Set the option to `false` to restore the previous behavior of logging silently. (by [@alexander-akait](https://github.com/alexander-akait) in [#600](https://github.com/webpack/less-loader/pull/600))

### Minor Changes

- Added types. (by [@alexander-akait](https://github.com/alexander-akait) in [#595](https://github.com/webpack/less-loader/pull/595))

### Patch Changes

- Track files loaded synchronously by Less (e.g. `data-uri()` and custom functions installed via `@plugin`) as webpack file dependencies. See [#492](https://github.com/webpack/less-loader/issues/492). (by [@alexander-akait](https://github.com/alexander-akait) in [#596](https://github.com/webpack/less-loader/pull/596))
